...
public class DeveloperController {
    ...
    public DeveloperResponse getById(@PathVariable("id") int id){
        ...
        return new DeveloperResponse(foundDeveloper, HttpStatus.OK.value(), "Search başarılı");
    }
    @PutMapping("/{id}")
    public DeveloperResponse updat(@PathVariable("id") int id, @RequestBody Developer developer){
        developer.setId(id);
    ...
    public DeveloperResponse delete(@PathVariable("id") int id){
        Developer removerDeveloper = this.developers.get(id);
        this.developers.remove(id);
        return new DeveloperResponse(removerDeveloper, HttpStatus.NO_CONTENT.value(), "silme başarılı");
    }
}
